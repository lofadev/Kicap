...
const ShowSlideBanner = () => {
  ...
  const fetchData = async (payload) => {
    const res = await SliderService.getSliders(payload, user.accessToken, dispatch);
    if (res.status === 'OK') {
      setResponse(res);
  ...
  return (
    <div className='shippers'>
      ...
      <Box title='Danh sách slide banner'>
        ...
        <DataTable rows={rows} head={['Ảnh']} keys={keys} handleOpenDelete={handleOpenDelete} />
        <Pagination
