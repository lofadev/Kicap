...
const ProductDetails = () => {
  ...
  const handleFillterRelatedProduct = useCallback(() => {
    return products.filter((product) => product.type === productState.type && product.id !== productState.id);
  }, [productState]);
  ...
  const handleCalculatorPrice = useCallback(
    (index) => {
      return productState.price[index] - (productState.price[index] * productState.discount[index]) / 100;
    },
    [productState]
  );
  ...
  const fomattedOldPrice = useMemo(() => oldPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") + "đ", [oldPrice]);
  ...
  }, [title, productState, handleFillterProduct, handleFillterRelatedProduct, handleCalculatorPrice]);
  ...
  return (
    <>
      ...
      <section className="product">
                ...
                <div className="product-top">
                  ...
                  <div className="inventory_quantity">
                    ...
                    <span className="a-stock">
                      {status === 0 ? "Hết hàng" : status > 0 ? `${status} sản phẩm có sẵn` : "Hàng order"}
                    </span>
                  </div>
                  ...
                  <div className="form-product">
                    <div id="add-to-cart-form">
                      ...
                      <div className="select-swatch">
                        {productState.layout && <SwatchSelect product={productState} field="layout"></SwatchSelect>}
                        {productState.species && <SwatchSelect product={productState} field="species"></SwatchSelect>}
                        {productState.color && <SwatchSelect product={productState} field="color"></SwatchSelect>}
                        {productState.switch && <SwatchSelect product={productState} field="switch"></SwatchSelect>}
                      </div>
                        ...
                        {(status === null || status > 0) && (
                          <div className="btn-buy">
                            <button className="btn-cart add_to_cart">
                              <span className="txt-main">Mua ngay với giá {formattedSpecialPrice}</span>
        ...
        <SectionProduct max="4" products={relatedProduct} title="sản phẩm" strongTitle="liên quan"></SectionProduct>
      </section>
    </>
  );
};
